...
location = '/home/016080064/waymo-open-dataset/cnn/'
...
cnn = cnnLayer()
opt = optim.Adagrad(cnn.parameters(), lr=0.0001)
...
for steps in tqdm(range(training)):
    ...
    for file in files:
        ...
        for i, (_, r) in enumerate(cam_hkp.iterrows()): 
            ...
            if (batch and c==32) or (batch == 0 and c == (remainder)):
                out = cnn(torch.stack(images).to(torch.float))
    ...
    # if steps%10 == 0:
    torch.save(cnn, location + f'/models/model-{steps}.pt')
